...
joined_data <- rbind(df_3_1 %>% select(lemma_txt, overall_id) %>% 
                       rename( description = lemma_txt, doc_id = overall_id),
...
df_3_1Length <- nrow(joined_data %>% filter(doc_id < 100000))

escoLength <- nrow(joined_data %>% filter(doc_id > 100000))
...
for (i in 1: df_3_1Length)
{
  ...
  frase <- df_3_1$original_txt[i] # taking the original sentence without text cleaning op from moocs datasets
...
similarities <- cbind(similarities, df_3_1 %>% select(original_txt, overall_id))
...
similarities_1 <- similarities %>%
  ...
  mutate(doc_id_esco = if_else(cosine_similarity > 0.43, doc_id_esco, "")) %>%
...
skills_dataset <- df_3_1 %>%
  left_join(similarities_1 %>% select(skill, overall_id, doc_id_esco) %>% rename(skills_list = skill),
            by = ("overall_id")) %>% 
  mutate(doc_id_esco = as.numeric(doc_id_esco)) %>% 
  mutate(doc_id_esco = doc_id_esco - 100000) %>%  # return to doc id esco original value
  mutate(doc_id_esco = as.character(doc_id_esco))
