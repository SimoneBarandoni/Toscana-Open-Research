...
df <- df %>% 
  mutate(descr_clean = str_replace_all(descr_clean,
                                       case_insensitive("e") %R% or(ANY_CHAR, SPC) %R% case_insensitive("commerce"),
                                                           "ecommerce")) %>% 
  mutate(descr_clean = str_remove_all(descr_clean, whole_word("così")))

df <- df %>% 
  mutate(skill_clean = str_replace_all(skill_clean,
                                       case_insensitive("e") %R% or(ANY_CHAR, SPC) %R% case_insensitive("commerce"),
                                                     "ecommerce")) %>% 
  mutate(skill_clean = str_remove_all(skill_clean, whole_word("così")))
