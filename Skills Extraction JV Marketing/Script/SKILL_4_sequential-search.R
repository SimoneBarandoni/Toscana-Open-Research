...
df_1 <- df_1 %>% 
  mutate(original_txt = word) %>%
  mutate(word = str_to_lower(word)) %>%
  mutate(word = str_remove_all(word, pat_agg)) %>% 
  mutate(word = str_replace_all(word, one_or_more(SPC), " ")) %>% 
  mutate(word = str_remove_all(word, or(START %R% SPC, SPC %R% END)))
...
for (frase in 1:nrow(df_1))
{
  print(str_c("papers done ----->  ", round(frase/nrow(df_1), digits = 4)*100, "%"))
  
  subset <- esco_original_1_multiple_word  
  
  vet <- unlist(str_split(df_1[[frase,"word"]], " "))
  ...
  while (competenza <= nrow(subset))
  {
    ...
    for (i in 1:length(comp))
    {
      ...
      {
        df_1[[frase,"skill"]] <- paste(df_1[[frase,"skill"]],subset[[competenza,"original_txt"]],sep = " ; ")
        df_1[[frase,"doc_id_esco"]] <- paste(df_1[[frase,"doc_id_esco"]],subset[[competenza,"doc_id_esco"]],sep = " ; ")
        id_remove <- subset[[competenza,"id"]]
        subset <- subset %>% 
          filter(id!=id_remove)
        shift <- TRUE
...
write_rds(df_1,"Intermediate/df_sequential_search.rds")
